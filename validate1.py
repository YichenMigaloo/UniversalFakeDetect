...
import random
import shutil
from scipy.ndimage import gaussian_filter  # Updated import to avoid DeprecationWarning
...
class RealFakeDataset(Dataset):
    def __init__(self, real_path, fake_path, data_mode, max_sample, arch, jpeg_quality=None, gaussian_sigma=None):
        ...
        if len(real_list) == 0 or len(fake_list) == 0:
            print("No images found in the provided paths.")
            self.total_list = []
        else:
            self.total_list = real_list + fake_list

        self.labels_dict = {i: 0 for i in real_list}
        self.labels_dict.update({i: 1 for i in fake_list})
    ...
    def read_path(self, real_path, fake_path, data_mode, max_sample):
        ...
        if max_sample is not None:
            if (max_sample > len(real_list)) or (max_sample > len(fake_list)):
                ...
                print("Not enough images, max_sample falling to", max_sample)
...
if __name__ == '__main__':
    ...
    for dataset_path in dataset_paths:
        ...
        print(dataset_path['real_path'], dataset_path['fake_path'])
